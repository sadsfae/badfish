...
class TestAsyncioFix(unittest.TestCase):
    ...
    def test_main_handles_no_event_loop(self, mock_get_loop, mock_new_loop, mock_set_loop,
                                        mock_parse_args, mock_logger, mock_execute):
        mock_get_loop.side_effect = RuntimeError("There is no current event loop in thread 'MainThread'.")
        ...
        mock_parse_args.return_value = {
            ...
            "firmware_inventory": None, "host_list": None, "log": None, "output": None
        }
    ...
    @patch('asyncio.new_event_loop')
    @patch('asyncio.get_event_loop')
    def test_main_uses_existing_loop(self, mock_get_loop, mock_new_loop, mock_set_loop,
                                     mock_parse_args, mock_logger, mock_execute):
